import { getStripe, getSiteUrl } from '../../../api/lib/stripe-client.js'
import { createPendingOrder } from '../../../api/lib/orders.js'
import { computeQuote, getPack } from '../../../api/lib/packs.js'
...
export default async function handler(req, res) {
  ...
  try {
    const { pack, faceCount, email, customerName } = req.body ?? {}
    if (!pack) return res.status(400).json({ error: 'pack requis (duo | famille | grande_famille)' })
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${site}/pipeline/paiement/reussi?order=${order.access_token}`,
