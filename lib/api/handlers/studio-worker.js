...
import {
  ...
  releaseMotorLock,
  renewMotorLock,
  resolveWorkerFromRequest,
} from '../../server/motor-lock.js'
...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'POST') {
      ...
      const worker = resolveWorkerFromRequest(req, body.worker)
      const action = body.action

      if (action === 'claim_next') {
        const result = await claimNextMotorJob(req, {
          weekKey: body.weekKey ? String(body.weekKey) : null,
          worker,
          skipOrderIds: Array.isArray(body.skipOrderIds) ? body.skipOrderIds : [],
        })
      ...
      if (action === 'claim_job') {
        ...
        const result = await claimMotorLock(getSupabase(), orderId, worker)
        return res.status(200).json({ ok: result.claimed, ...result })
      }

      if (action === 'release_job') {
        const orderId = body.orderId ? String(body.orderId) : null
        const workerId = body.workerId ? String(body.workerId) : worker.id
        if (!orderId) return res.status(400).json({ error: 'orderId requis' })
        const result = await releaseMotorLock(getSupabase(), orderId, workerId)
        return res.status(200).json({ ok: true, ...result })
      }
      ...
      if (action === 'renew_job') {
        const orderId = body.orderId ? String(body.orderId) : null
        if (!orderId) return res.status(400).json({ error: 'orderId requis' })
        const result = await renewMotorLock(getSupabase(), orderId, worker)
        return res.status(200).json({ ok: result.renewed, ...result })
      }
    ...
    if (req.query?.identity === '1') {
      const worker = resolveWorkerFromRequest(req, null)
      return res.status(200).json({ ok: true, ip: worker.ip, suggestedLabel: worker.label })
    }
    ...
    return res.status(200).json({
      ...
      laserErrors: board.laserErrors,
      generationErrors: board.generationErrors,
      totalErrors: board.totalErrors,
    })
  } catch (e) {
