...
export default async function handler(req, res) {
  ...
  try {
    if (event.type === 'checkout.session.completed') {
      ...
      const orderId = sessionRaw.metadata?.order_id
      if (!orderId) {
        console.warn('[stripe-webhook] session sans order_id', sessionRaw.id)
        return res.status(200).json({ received: true })
      }

      // Expand invoice to get PDF URL when available
      let session = sessionRaw
      if (sessionRaw.invoice && typeof sessionRaw.invoice === 'string') {
        try {
