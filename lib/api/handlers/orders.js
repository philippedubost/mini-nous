...
import { sendLineartReadyEmailIfNeeded } from '../../server/order-email.js'
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'PATCH') {
      const { token, generationId, action, faceCount, characters, approvedPersons } = getJsonBody(req)
      if (!token) return res.status(400).json({ error: 'token requis' })
      ...
      if (action === 'pending_validation') {
        ...
        sendLineartReadyEmailIfNeeded(req, {
          ...
          sourcePhotoUrl: publicAssetUrl(req, rawPhoto ?? order.metadata?.paywall_source_url),
