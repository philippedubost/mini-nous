...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      return res.status(200).json({ order: await buildOrderResponse(req, order) })
    }

    if (req.method === 'PATCH') {
      const { token, generationId, action } = req.body ?? {}
      if (!token) return res.status(400).json({ error: 'token requis' })
      ...
      if (action === 'regen') {
        ...
        await supabase
          .from('mini_nous_orders')
          .update({
            metadata: { ...(order.metadata ?? {}), regen_count: count + 1 },
            workflow_status: WORKFLOW_STATUS.IN_STUDIO,
            updated_at: new Date().toISOString(),
          })
          .eq('id', order.id)
        return res.status(200).json({ ok: true, regenCount: count + 1, regenRemaining: MAX_REGEN - count - 1 })
      }
