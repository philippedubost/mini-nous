...
/** Routes lourdes (trace / binaires) — import dynamique pour ne pas casser tout /api sur Vercel. */
const LAZY_ROUTES = {
  'trace-autotrace': () => import('./handlers/trace-autotrace.js'),
  'studio-laser': () => import('./handlers/studio-laser.js'),
}
...
export const routes = {
  ...
  track,
  metrics,
  ...Object.fromEntries(Object.keys(LAZY_ROUTES).map(k => [k, true])),
}

export function hasRoute(name) {
  return !!(routes[name] || LAZY_ROUTES[name])
}

export async function resolveHandler(name) {
  if (routes[name] && routes[name] !== true) return routes[name]
  const loader = LAZY_ROUTES[name]
  if (!loader) return null
  if (!lazyHandlerCache.has(name)) {
    lazyHandlerCache.set(name, loader().then(m => m.default))
  }
  return lazyHandlerCache.get(name)
}
