...
export function chainStudioTick(orderId, delayMs = 1500) {
  ...
  const run = () => fetch(`${site}/api/studio-generate`, {
    method: 'POST',
    headers: {
  ...
  }).catch(err => console.error('[studio-generate chain]', err))
...
export async function runStudioTickForOrderId(req, orderId) {
  ...
  if (result.needsContinue) chainStudioTick(orderId)
  return result
}
