...
export async function workerClearErrors(orderId) {
  const supabase = getSupabase()
  const order = await loadOrder(supabase, orderId)
  const meta = { ...(order.metadata ?? {}) }
  ...
  const sg = meta.studio_generate
  if (sg?.phase === 'error' || sg?.error || (sg?.errorLog?.length ?? 0) > 0) {
    meta.studio_generate = {
      mode: sg.mode ?? 'initial',
      updatedAt: now,
    }
    cleared.push('fal')
  }

  if (meta.studio_laser) {
    delete meta.studio_laser
    cleared.push('laser')
  }
  ...
  if (order.generation_id) {
    ...
    if (gen?.status === 'error') {
      const { data: step2 } = await supabase
        ...
        .eq('asset_type', 'step2')
        .maybeSingle()
      const hasStep2 = !!(step2?.image_url || step2?.fal_url)
      const { error: patchErr } = await supabase
        .from('mini_nous_generations')
        .update({
          status: hasStep2 ? 'done' : 'running',
          error_message: null,
          updated_at: now,
        })
        .eq('id', order.generation_id)
...
export async function workerLaunchLaser(orderId) {
  return { orderId, clientOnly: true, reason: 'studio_worker_browser' }
}
    ...
    try {
      ...
      switch (action) {
        ...
        case 'launch_trace_v1':
          ...
          break
        case 'launch_laser':
          result = { orderId, ok: true, ...(await workerLaunchLaser(orderId)) }
          break
        case 'validate_trace':
        ...
        case 'mark_received':
          ...
          break
        case 'clear_errors':
          result = { orderId, ok: true, ...(await workerClearErrors(orderId)) }
          break
        default:
