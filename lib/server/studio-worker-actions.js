...
import { queueStudioLaser } from './studio-laser.js'
...
export async function workerValidateTrace(req, orderId) {
  ...
  const { error } = await supabase
    ...
    .eq('id', orderId)
  if (error) throw new Error(error.message)
  await queueStudioLaser(orderId).catch(err => console.error('[validate_trace laser]', err))
  return { workflowStatus: WORKFLOW_STATUS.APPROVED }
}

export async function workerLaunchLaser(orderId, { force = false } = {}) {
  return queueStudioLaser(orderId, { force })
}
    ...
    try {
      ...
      switch (action) {
        ...
        case 'launch_trace_v1':
          ...
          break
        case 'launch_laser':
          result = { orderId, ok: true, ...(await workerLaunchLaser(orderId, { force: true })) }
          break
        case 'validate_trace':
