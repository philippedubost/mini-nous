...
export function resolveServerColumn(order, { assets, lineartVersions, generation, job }) {
  ...
  if (ws === WORKFLOW_STATUS.REVISION_REQUESTED) {
    return 'step1_done_step2'
  }

  if (job && ACTIVE_JOB_PHASES.has(phase)) {
    if (job.mode === 'regen' || phase === 'step2' || job?.step2RequestId) {
      return 'step1_done_step2'
    }
    return 'order_step1'
  }
...
export async function buildStudioOrderCard(req, order, {
  ...
  const card = {
    ...
    motorLock: meta.motor_lock ?? null,
  }
