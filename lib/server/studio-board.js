...
function cardHasFalError(card) {
  const laserErr = card.column === 'validated_fabrication' && !card.hasLaserSvg
    && card.studioLaser?.phase === 'error'
  return !!(
    laserErr
    || card.canRetry && card.column !== 'validated_fabrication'
...
export function resolveServerColumn(order, { assets, lineartVersions, generation, job }) {
  ...
  if (ws === WORKFLOW_STATUS.IN_PRODUCTION || ws === WORKFLOW_STATUS.APPROVED) {
    if (order.status !== 'paid') return 'photo_payment'
    const hasTrace = !!(assets?.step2 || (lineartVersions?.length ?? 0) > 0)
    if (!hasTrace) {
      return assets?.step1 ? 'step1_done_step2' : (orderHasPhoto(order, assets) ? 'order_step1' : 'photo_payment')
    }
