...
function cardHasFalError(card) {
  return !!(
    card.canRetry
    || card.studioJob?.phase === 'error'
    || card.studioJob?.error
...
function buildErrorLog(job, generationError) {
...
export async function buildStudioOrderCard(req, order, { supabase, assetsMap, generationsMap, lineartCountsMap, detail = false }) {
  const meta = order.metadata ?? {}
  ...
  const errorLog = buildErrorLog(job, generation?.error_message ?? null)
  ...
  const card = {
    ...
    studioJob: job
      ...
      : errorLog.length
        ? { phase: 'error', error: errorLog[errorLog.length - 1]?.message ?? null, errorLog }
        : null,
  ...
  return {
    ...
    isBlocked24h: isBlocked24h(enriched),
    stuckHours: Math.floor(cardStuckMs(enriched) / (60 * 60 * 1000)),
  }
}
...
export async function buildStudioBoard(req, { weekKey = null } = {}) {
  ...
  const [assetsMap, generationsMap, lineartCountsMap] = await Promise.all([
  ...
  const cards = await Promise.all(
    orders.map(order => buildStudioOrderCard(req, order, {
      supabase, assetsMap, generationsMap, lineartCountsMap, detail: false,
    })),
  )
  ...
  for (const card of kanbanCards) {
    if (columns[card.column]) {
      ...
      if (card.hasFalError) columnTotals[card.column].errors += 1
...
export async function buildStudioOrderDetail(req, orderId) {
  ...
  const card = await buildStudioOrderCard(req, order, {
    supabase, assetsMap, generationsMap, lineartCountsMap, detail: true,
  })
