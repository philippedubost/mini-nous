...
import { hasAdminAccess, isAdminEmail, normalizeEmail } from './admin.js'
import {
...
/** Admin connecté sur sa propre commande → vue client (validation tracé incluse). */
function isOrderOwner(order, authUser) {
  if (!authUser || !order) return false
  if (order.user_id && authUser.id && order.user_id === authUser.id) return true
  if (order.email && authUser.email) {
    return normalizeEmail(order.email) === normalizeEmail(authUser.email)
  }
  return false
}
...
export async function buildOrderResponse(req, order, authUser = null) {
  ...
  const studioJobActive = !!(
    sg && !['done', 'error', 'awaiting_photo'].includes(sg.phase)
  )
  ...
  const isAdminView = admin && !isOrderOwner(order, authUser)
  ...
  const studio = getStudioCaps(order.metadata, workflowStatus, isAdminView, lineartVersions.length)
  const clientLineartUrl = studioJobActive ? null : (lineartUrl || null)
  const resolvedPreview = clientLineartUrl
  ...
  const payload = buildCustomerOrderPayload({
    ...
    lineartUrl: publicAssetUrl(req, clientLineartUrl),
    sourcePhotoUrl: publicAssetUrl(req, sourcePhotoUrl),
    siteUrl: getSiteUrl(req),
  })
  const editable = order.status === 'paid' && (isAdminView || isOrderEditable({
    workflowStatus,
    generation,
    week: order.week,
  }))
  const paywallPhotoReplacements = Number(order.metadata?.paywall_photo_replacements) || 0
  const studioGenerateActive = !!(studioJobActive && order.status === 'paid')
  return {
    ...payload,
    editable,
    regenCount,
    ...
    studioGenerateMode: sg?.mode ?? null,
  }
}
