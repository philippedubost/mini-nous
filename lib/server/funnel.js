 ...
 *   paywall_opened       — landing /commander (WoodTribe) ou /mininous/commander
 *   studio_opened        — page commande / studio client
 */
export async function trackFunnelEvent(event, {
  ...
  try {
    ...
    const { error } = await supabase.from('mini_nous_funnel_events').insert({
      event,
    ...
    if (error) console.warn('[funnel]', event, error.message)
  } catch (err) {
