...
import { ensureOrderKanbanThumb } from './kanban-thumb.js'
...
async function attachPaywallPhoto(env, order, photoBase64, { countAsReplacement = false } = {}) {
  ...
  const kanbanThumbUrl = parsed?.buf
    ? await ensureOrderKanbanThumb(env, supabase, order.id, parsed.buf)
    : null
  ...
  const { data: updated, error } = await supabase
    ...
    .update({
      metadata: {
        ...
        ...(kanbanThumbUrl ? { kanban_thumb_url: kanbanThumbUrl } : {}),
