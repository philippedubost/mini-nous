...
export const EXTRA_PERSON_CENTS_TRIO = 500
...
export function formatQuoteForClient(quote, { fridayDelivery = false } = {}) {
  ...
  const frPortCents = computePortCents(fc, 'fr')
  const intlPortCents = computePortCents(fc, 'international')

  let pricingNote = null
  if (quote.extraCount > 0 && fc !== 3) {
  ...
  return {
    ...
    shippingFrEur: (frPortCents / 100).toFixed(2),
    shippingIntlEur: (intlPortCents / 100).toFixed(2),
