...
const TEST_EMAIL = 'philippe@lesfantomes.fr'
...
export function assertTestFlowAccess(req) {
  if (!isTestFlowEnabled() && !hasAdminHeader(req)) {
    throw Object.assign(new Error('Test flow désactivé — mot de passe admin requis'), { status: 404 })
  }
...
async function loadPlaceholderPhotoBase64(req) {
  const paths = [
  ...
  const res = await fetch(`${site}/images/duo.webp`)
  if (res.ok) {
  ...
  throw new Error('Image placeholder images/duo.webp introuvable (disque et URL publique)')
}
...
export async function createTestDraftOrder(req, env, {
  ...
  const photoBase64 = await loadPlaceholderPhotoBase64(req)
