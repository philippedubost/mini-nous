...
const BRAND = 'WoodTribe'

function emailBrandHeader() {
  return `<p style="font-size:18px;font-weight:700;color:#c0684a">${BRAND}</p>`
}

function emailBrandFooter() {
  return `${BRAND} · Nantes`
}
...
<body style="font-family:system-ui,sans-serif;line-height:1.6;color:#2c1f14;max-width:520px;margin:0 auto;padding:24px">
  ...
  <p style="font-size:12px;color:#9a8b7e;margin-top:32px">Conservez cet e-mail pour retrouver votre commande.<br/>${emailBrandFooter()}</p>
</body>
</html>`
}
...
export async function sendOrderConfirmationEmailIfNeeded(req, {
  ...
  const from = process.env.EMAIL_FROM || 'WoodTribe <commandes@woodtribe.fr>'
  if (!apiKey) {
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from,
      to: [email],
      subject: friendlyEmailSubject(firstName, 'ta commande WoodTribe est confirmée ✨'),
      html: buildConfirmationHtml({
        customerFirstName: firstName,
        customerName,
        packLabel: packLabel || 'WoodTribe',
...
<body style="font-family:system-ui,sans-serif;line-height:1.6;color:#2c1f14;max-width:520px;margin:0 auto;padding:24px">
  ...
  <p style="font-size:12px;color:#9a8b7e;margin-top:32px">${emailBrandFooter()}</p>
</body>
</html>`
}
...
<body style="font-family:system-ui,sans-serif;line-height:1.6;color:#2c1f14;max-width:520px;margin:0 auto;padding:24px">
  ${emailBrandHeader()}
  <p>${greeting}</p>
  <p>Bonne nouvelle — la fabrication de vos figurines est terminée à l'atelier.</p>
  ${dualPreviewHtml(sourcePhotoUrl, previewUrl)}
...
<body style="font-family:system-ui,sans-serif;line-height:1.6;color:#2c1f14;max-width:520px;margin:0 auto;padding:24px">
  ${emailBrandHeader()}
  <p>${greeting}</p>
  <p>Vos figurines sont en route — le colis a quitté notre atelier à Nantes.</p>
...
export async function sendLineartReadyEmailIfNeeded(req, {
  ...
  const body = await sendResendEmail({
    ...
    to: [email],
    subject: v === 1
      ? friendlyEmailSubject(firstName, 'besoin de ta validation pour tes figurines 🔍')
...
export async function sendFabricationCompleteEmailIfNeeded(req, {
  ...
  const body = await sendResendEmail({
    ...
    from,
    to: [email],
    subject: friendlyEmailSubject(firstName, 'tes figurines sont prêtes à l\'atelier ✂️'),
...
export async function sendShippedEmailIfNeeded(req, {
  ...
  const body = await sendResendEmail({
    ...
    from,
    to: [email],
    subject: friendlyEmailSubject(firstName, 'ton colis WoodTribe est en route 📦'),
