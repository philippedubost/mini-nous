...
export async function loadOrderContext(supabase, order) {
  if (!order) return { generation: null, previewUrl: null }
  ...
  if (generation?.id) {
    ...
    previewUrl = stepMap.step2 || stepMap.step1 || stepMap.source || null
  }

  return { generation, previewUrl }
}
...
export function buildCustomerOrderPayload({ order, generation, previewUrl, siteUrl }) {
  ...
  return {
    ...
    links: {
      ...
      studio: `${site}/pipeline/studio?order=${encodeURIComponent(token)}`,
      shop: site,
    },
  }
}
