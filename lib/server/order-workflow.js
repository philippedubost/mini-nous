...
export const WORKFLOW_META = {
  ...
  revision_requested: {
    ...
    hint: 'Notre équipe reprend votre design selon vos suggestions.',
    step: 2,
  },
  approved: {
    label: 'Design validé',
    hint: 'Votre design est validé — il part en file d\'impression.',
    step: 3,
  },
  in_production: {
    label: 'En fabrication',
    hint: 'Vos figurines sont en cours de découpe et finition à Nantes.',
    step: 3,
  },
  shipped: {
    label: 'Expédié',
...
export function computeWorkflowStatus({ order, generation, week }) {
  ...
  if (stored === WORKFLOW_STATUS.APPROVED && !generation?.fabricated_at) {
...
export function isOrderEditable({ workflowStatus, generation, week }) {
  if (generation?.fabricated_at) return false
  if (week?.status === 'in_production') return false
  return EDITABLE_STATUSES.has(workflowStatus)
}

export async function loadOrderContext(supabase, order) {
  if (!order) return { generation: null, previewUrl: null, sourcePhotoUrl: null }
  ...
  if (generationId) {
    const { data } = await supabase
      ...
      .select('id, status, fabricated_at, fabricated_week_id')
      .eq('id', generationId)
      .maybeSingle()
    generation = data
  }
  ...
  if (generation?.id) {
    ...
    sourcePhotoUrl = stepMap.source || null
    previewUrl = stepMap.step2 || stepMap.step1 || null
  }

  return { generation, previewUrl, sourcePhotoUrl }
}
...
export function buildCustomerOrderPayload({ order, generation, previewUrl, sourcePhotoUrl, siteUrl }) {
