...
export async function loadOrderContext(supabase, order) {
  ...
  lineartUrl = stepMap.step2 || null
    // Ne jamais exposer step1 (mise en scène photoréaliste) au client — uniquement le line art step2
...
export function buildCustomerOrderPayload({
  ...
  const site = (siteUrl || process.env.SITE_URL || 'https://www.woodtribe.fr').replace(/\/$/, '')
  ...
  return {
    ...
    customerFirstName: resolveCustomerFirstName(order),
    ...
    links: {
      ...
      studio: `${site}/pipeline/commande?order=${encodeURIComponent(token)}`,
      shop: site,
    },
