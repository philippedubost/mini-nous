...
import {
  ...
  sendFabricationCompleteEmailIfNeeded,
} from './order-email.js'
...
export async function applyAdminWorkflowStatus(req, supabase, order, targetStatus) {
  ...
  const previewUrl = publicAssetUrl(req, rawPreview)
  const sourcePhotoUrl = publicAssetUrl(req, rawPhoto ?? meta.paywall_source_url)
  const lineartVersion = getLineartVersion(patchMeta)

  if (targetStatus === WORKFLOW_STATUS.APPROVED) {
    await sendDesignValidatedEmailIfNeeded(req, {
      orderId: order.id,
      email: order.email,
    ...
    }).catch(err => console.error('[admin-workflow validated-email]', err))
  }

  if (targetStatus === WORKFLOW_STATUS.IN_PRODUCTION) {
    await sendFabricationCompleteEmailIfNeeded(req, {
      orderId: order.id,
      email: order.email,
      accessToken: order.access_token,
      packLabel: meta.pack_label,
    ...
    }).catch(err => console.error('[admin-workflow fabrication-email]', err))
  }
