...
import { markOrdersInProductionForGenerations } from './order-workflow.js'
...
export async function buildGenerationsBatch({
  ...
  if (week) {
    ...
    await markOrdersInProductionForGenerations(supabase, generationIdsFromPlacements(placements))
...
export async function buildWeekBatch({ weekKey, dryRun = false, kerfMm = -0.1 } = {}) {
  ...
  await markOrdersInProductionForGenerations(getSupabase(), generationIdsFromPlacements(placements))
  return { ...result, uploaded: true, batchSvgUrl: url, batchR2Key: key, builtAt }
}
