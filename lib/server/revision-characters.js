export const ISSUE_OPTIONS = [
  'cheveux', 'habits', 'expression', 'age_jeune', 'age_vieux',
  'proportions', 'posture', 'visage', 'vetements', 'accessoires',
]
...
export function sanitizeRevisionCharacters(characters) {
  ...
  return characters.map((c, i) => ({
    ...
    issues: (c.issues ?? []).filter(x => ISSUE_OPTIONS.includes(x)).slice(0, 10),
